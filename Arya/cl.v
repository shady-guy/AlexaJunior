  ...
  always@(posedge clk or negedge clr) begin //OD
    if(!clr) begin
      ...
      case(cur)
        ...
        init    : begin acc<=0; out_idx<=0; k_idx<=0; end
        mac     : begin acc <= acc + input[out_idx + k_idx] * kernel[k_idx]; k_idx<=k_idx+1; end
